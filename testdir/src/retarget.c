...
int32_t SH_DoCommand(int32_t n32In_R0, int32_t n32In_R1, int32_t *pn32Out_R0)
{
    __ASM volatile ("BKPT   0xAB");          //("MRS %0, control" : "=r" (result) );
    //; ICE will step over BKPT directly
    //; HardFault will step BKPT and the next line
    asm("B      SH_ICE");

	SH_HardFault:             //; Captured by HardFault
    asm("MOVS   R0, #0");        //; Set return value to 0
    asm("BX     lr );           //; Return
    ...
    asm("CMP    R2, #0);
    asm("BEQ    SH_End);
    asm("STR    R0, [R2] );     //; Save the return value to *pn32Out_R0

		SH_End:
    asm("MOVS   R0, #1  );      //; Set return value to 1
    asm("BX     lr);            //; Return
}
# endif 
...
int _write(int file, char *ptr, int len)
{
    ...
    {
        SendChar_ToUART(*ptr++);
        //SH_DoCommand(0x04, *ptr++, NULL);
    }
    return len;
	#else
		_swiwrite(file, ptr, len);
	#endif
